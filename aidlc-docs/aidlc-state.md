...
- **Current Stage**: OPERATIONS - deployment PREPARED, PAUSED. Next: execute the Azure build from `private/deployment-runbook.md` (gitignored, local only) at the next training.
- **Session note (2026-07-24T17:25:00Z)**: paused after preparing the deployment. Done this session: W-1..W-6 (settings.py env config, asgi.py entry point, provision.py account CLI, doc fixes, systemd + nginx artifacts under deploy/, runbook); FIXED A BLOCKING DEFECT - `alembic upgrade head` failed on a fresh DB because revision 0001 create_all's the live schema and 0002 re-created accounts; 0002 is now idempotent and tests/data_management/test_migrations.py covers the path no gate exercised; alembic/env.py now reads AIDLC_DATABASE_URL. SECURITY: the deployment plan and runbook were moved to a gitignored `private/` directory and purged from public git history (they inventory the configuration's weaknesses); an earlier accidental publication of another system's allowlisted IPs was also purged. Gates: pytest 193, mypy 113, ruff, lint-imports 14; frontend tsc/eslint/vitest 12. Original plan detail: deployment W-1..W-6 IMPLEMENTED (settings/asgi/provision CLI/doc fixes/deploy artifacts/runbook), gates green (pytest 190, mypy 112, ruff, lint-imports 14; frontend tsc/eslint/vitest 12). Actual Azure work to be done per deployment-runbook.md. Original plan: (Azure VM / pseudonymized data / self-hosted proxy+Let's Encrypt / SQLite / systemd x2 / env vars / manual deploy / provisioning CLI). Awaiting approval to implement W-1..W-6. Blockers to fix: G-1 env config, G-2 ASGI entry point (README/DEVELOPMENT quick-start currently WRONG), G-3 provisioning CLI. New handoff DEP-H1: self-hosted proxy re-opens H-6 (no WAF; access logging must be enabled on our proxy for SECURITY-02).
- **Session note (2026-07-24T12:00:00Z)**: user paused. Everything through Operations docs is complete and pushed to github.com/u-10bei/AI-DLC_test (public, main, commit 815f641). Next training: production deployment. Pre-prod handoffs (see DEVELOPMENT.md / aidlc-state handoffs): set trusted_proxies, encrypted volume + audit chattr +a + rotation cron, OS-level account provisioning (U06-H5), pip-audit/SBOM in CI (SECURITY-10), build & serve frontend (npm run build feeds U08-H4 static mount), SQLite->PostgreSQL via connection string. Deferred features remain: U05-H6, U08-H2/H3/H6, US-06/10/12/25.
